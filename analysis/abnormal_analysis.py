"""
Analyze abnormal experiment outputs (QUIC/MPQUIC/TCP/MPTCP).
...
LOG_ROOT_BASE = Path("./logs/abnormal")
DEFAULT_LOG_DIR_NAME = Path("default")
OUTPUT_ROOT = Path("./analysis/plots/abnormal")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Analyze abnormal experiment logs.")
    parser.add_argument(
        "--log-dir",
        dest="log_dir_name",
        type=Path,
        default=DEFAULT_LOG_DIR_NAME,
        help="Directory name under logs/abnormal to analyze (default: default).",
    )
    parser.add_argument(
        "--output-dir",
        ...
        help="Output subdirectory name under analysis/plots/abnormal (default: default).",
    )
    parser.add_argument(
        "--elephant-num",
        action="append",
        ...
        help=(
            ...
            "Default: 4 (abnormal_loss.py default)."
        ),
    )
    parser.add_argument(
        "--loss-rate",
...
def main() -> None:
    ...
    for elephant_num, kind, value in sorted(combos, key=lambda x: (x[0], x[1], x[2])):
        ...
        plot_goodput_violin_proto(
            elephant_combo,
            combo_output_dir / "abnormal_elephant_goodput_violin.png",
            PROTO_ORDER,
            proto_colors=PROTO_COLORS,
        ...
        plot_mouse_fct_cdf(
            mouse_combo,
            combo_output_dir / "abnormal_mouse_fct_cdf.png",
            PROTO_ORDER,
            mark_outliers=True,
        )
        plot_mouse_fct_cdf(
            mouse_combo,
            combo_output_dir / "abnormal_mouse_fct_cdf_no_outliers.png",
            PROTO_ORDER,
            exclude_outliers=True,
        )
        plot_run_p99_scatter(
            mouse_combo,
            combo_output_dir / "abnormal_mouse_p99_scatter.png",
            PROTO_ORDER,
        )
        if args.heatmap_mode == "pivot":
            plot_link_heatmap(
                link_combo,
                combo_output_dir / "abnormal_link_heatmap.png",
                PROTO_ORDER,
            )
        else:
