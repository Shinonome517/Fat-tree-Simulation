"""
Analyze incast experiment outputs (QUIC vs MPQUIC).

This script scans the incast log directory, aggregates elephant goodput,
...
LOG_ROOT_BASE = Path("./logs/incast")
DEFAULT_LOG_DIR_NAME = Path("default")
OUTPUT_ROOT = Path("./analysis/plots/incast")
PROTO_ORDER = ("quic", "mpquic")
MOUSE_DROPLOSS_FILENAME = "incast_mouse_droploss_ratio.png"
MOUSE_RETRANS_FILENAME = "incast_mouse_retrans_ratio.png"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Analyze incast experiment logs.")
    parser.add_argument(
        "--log-dir",
        dest="log_dir_name",
        type=Path,
        default=DEFAULT_LOG_DIR_NAME,
        help="Directory name under logs/incast to analyze (default: default).",
    )
    parser.add_argument(
        "--output-dir",
        ...
        help="Output subdirectory name under analysis/plots/incast (default: default).",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
...
def main() -> None:
    ...
    plot_fct_cdf(
        mouse_df,
        output_dir / "incast_fct_cdf.png",
        PROTO_ORDER,
        mark_outliers=True,
    )
    plot_fct_cdf(
        mouse_df,
        output_dir / "incast_fct_cdf_no_outliers.png",
        PROTO_ORDER,
        exclude_outliers=True,
    )
    plot_fct_histogram(
        mouse_df,
        output_dir / "incast_fct_hist.png",
        PROTO_ORDER,
        mark_outliers=True,
    )
    plot_fct_histogram(
        mouse_df,
        output_dir / "incast_fct_hist_no_outliers.png",
        PROTO_ORDER,
        exclude_outliers=True,
    )
    ...
    if args.heatmap_mode == "pivot":
        plot_link_heatmap(
            link_df,
            output_dir / "incast_link_heatmap.png",
            PROTO_ORDER,
        )
    else:
