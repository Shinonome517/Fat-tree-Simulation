"""
Compute and plot the fraction of Mouse flows that retried because of drops.

For each mouse CSV (one flow), we look at retransmissions minus spurious
retransmissions. If the remaining retrans count is positive, that flow is
treated as having a drop-induced retry. The script aggregates the fraction of
such flows per run directory and emits a bar chart.
"""
...
def summarize_run(run_dir: Path, label: str | None = None) -> DropLossSummary:
    csv_paths = _list_mouse_csvs(run_dir)
    drop_flows = sum(1 for path in csv_paths if _flow_has_drop_retrans(path))
    return DropLossSummary(
...
def summarize_runs(run_dirs: Iterable[Tuple[str, Path]]) -> List[DropLossSummary]:
    summaries: List[DropLossSummary] = []
    for label, run_dir in run_dirs:
        summaries.append(summarize_run(run_dir, label))
    return summaries


def plot_drop_retrans_ratios(
    summaries: Sequence[DropLossSummary],
    ...
    if not filename:
        filename = f"mouse_droploss_{int(time.time())}.png"
    ...
    for bar, summary in zip(bars, summaries):
        ...
        text = f"{summary.drop_flows}/{summary.total_flows} ({ratio_pct:.1f}%)"
        ax.text(
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot fraction of Mouse flows with drop-induced retransmissions."
    )
    parser.add_argument(
        "--log-root",
        type=Path,
    ...
    parser.add_argument(
        "--proto",
        action="append",
        help="Protocol(s) to include (default: all subdirectories under log-root).",
    )
    parser.add_argument(
        "--run-id",
        action="append",
    ...
    parser.add_argument(
        ...
        action="append",
        type=Path,
        help="Explicit run directory; bypasses log-root scan when provided.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=DEFAULT_OUTPUT_DIR,
        help="Directory to store the generated plot (default: analysis/plots).",
    )
    parser.add_argument(
        "--output-name",
        default=None,
        help="Filename for the plot (default: autogenerated timestamped name).",
    )
    parser.add_argument(
        "--title",
        default=None,
...
def main() -> None:
    ...
    output_path = plot_drop_retrans_ratios(
        summaries,
        output_dir=args.output_dir,
        filename=args.output_name,
        title=args.title or "Mouse drop-induced retransmissions",
    )
    total_flows = sum(s.total_flows for s in summaries)
    drop_flows = sum(s.drop_flows for s in summaries)
    logging.info(
        "Saved drop-loss plot to %s (drop flows: %d/%d).",
        output_path,
        drop_flows,
        total_flows,
    )
